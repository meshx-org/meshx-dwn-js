...
import { anyJwk } from '../../../../src/models/jwk/jwk.js'
...
describe('GeneralJwk Schema', async () => {
    ...
    ;[jwkEd25519, jwkSecp256k1, jwkRsa].forEach((jwk): void => {
        it('should not throw an exception if properly formatted jwk', () => {
            expect(() => validateZodSchema(anyJwk, jwk.publicJwk)).to.not.throw()
            expect(() => validateZodSchema(anyJwk, jwk.privateJwk)).to.not.throw()
        })
    })
})
