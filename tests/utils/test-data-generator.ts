...
export class TestDataGenerator {
     ...
     */
    public static async generateProtocolsQueryMessage(
        input?: GenerateProtocolsQueryMessageInput
    ): Promise<GenerateProtocolsQueryMessageOutput> {
     ...
     */
    public static async generateCollectionsWriteMessage(
        input?: GenerateCollectionsWriteMessageInput
    ): Promise<GenerateCollectionsWriteMessageOutput> {
        const { requester, target } = await TestDataGenerator.generateRequesterAndTargetPersonas(input)
     ...
     */
    public static async generateCollectionsQueryMessage(
        input?: GenerateCollectionsQueryMessageInput
    ): Promise<GenerateCollectionsQueryMessageOutput> {
        const { requester, target } = await TestDataGenerator.generateRequesterAndTargetPersonas(input)
     ...
     */
    public static async generateHooksWriteMessage(
        input?: GenerateHooksWriteMessageInput
    ): Promise<GenerateHooksWriteMessageOutput> {
        const { requester, target } = await TestDataGenerator.generateRequesterAndTargetPersonas(input)
    ...
    public static async generatePermissionsRequestMessage(): Promise<{ target; message: BaseMessage }> {
        ...
        const permissionRequest = await PermissionsRequest.create({
            ...
            signatureInput: {
                ...
                protectedHeader: { alg: privateJwk.alg as string, kid: 'whatev' },
            },
        })
     ...
     * If `requester` and `target` are both not given, use the same persona to pass authorization in tests by default.
     */
    private static async generateRequesterAndTargetPersonas(input?: {
        requester?: Persona
        target?: Persona
        ...
        // if `requester` and `target` are both not given, use the same persona to pass authorization in tests by default
        if (input?.requester === undefined && input?.target === undefined) {
            requester = target
        }
