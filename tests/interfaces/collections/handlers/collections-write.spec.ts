...
describe('handleCollectionsWrite()', () => {
    ...
    describe('functional tests', () => {
        ...
        it('should only be able to overwrite existing record if new record has a later `dateCreated` value', async () => {
            ...
            const collectionsWriteMessageData = await TestDataGenerator.generateCollectionsWriteMessage({
                requester,
                target,
                data: data1,
            })
            ...
            const collectionsWriteReply = await handleCollectionsWrite(
                collectionsWriteMessageData.message,
                messageStore,
                didResolverStub
            )
            ...
            const collectionsQueryReply = await handleCollectionsQuery(
                collectionsQueryMessageData.message,
                messageStore,
                didResolverStub
            )
            expect(collectionsQueryReply.status.code).to.equal(200)
            expect(collectionsQueryReply.entries?.length).to.equal(1)
            expect((collectionsQueryReply.entries![0] as CollectionsWriteMessage).encodedData).to.equal(
                base64url.baseEncode(data1)
            )
            ...
            const newCollectionsWriteReply = await handleCollectionsWrite(
                newCollectionsWriteMessageData.message,
                messageStore,
                didResolverStub
            )
            ...
            const newCollectionsQueryReply = await handleCollectionsQuery(
                collectionsQueryMessageData.message,
                messageStore,
                didResolverStub
            )
            ...
            expect((newCollectionsQueryReply.entries![0] as CollectionsWriteMessage).encodedData).to.equal(
                base64url.baseEncode(data2)
            )
            ...
            // expecting unchanged
            const thirdCollectionsQueryReply = await handleCollectionsQuery(
                collectionsQueryMessageData.message,
                messageStore,
                didResolverStub
            )
            ...
            expect((thirdCollectionsQueryReply.entries![0] as CollectionsWriteMessage).encodedData).to.equal(
                base64url.baseEncode(data2)
            )
        })
        ...
        it('should only be able to overwrite existing record if new message CID is larger when `dateCreated` value is the same', async () => {
            ...
            const originatingMessageWriteReply = await handleCollectionsWrite(
                originatingMessageData.message,
                messageStore,
                didResolverStub
            )
            ...
            expect(collectionsQueryReply.status.code).to.equal(200)
            expect(collectionsQueryReply.entries?.length).to.equal(1)
            expect((collectionsQueryReply.entries![0] as CollectionsWriteMessage).descriptor.dataCid).to.equal(
                smallerCollectionWriteMessageData.message.descriptor.dataCid
            )

            // attempt to write the message with larger lexicographical message CID
            const newCollectionsWriteReply = await handleCollectionsWrite(
                largerCollectionWriteMessageData.message,
                messageStore,
                didResolverStub
            )
            expect(newCollectionsWriteReply.status.code).to.equal(202)
            ...
            expect(newCollectionsQueryReply.status.code).to.equal(200)
            expect(newCollectionsQueryReply.entries?.length).to.equal(1)
            expect((newCollectionsQueryReply.entries![0] as CollectionsWriteMessage).descriptor.dataCid).to.equal(
                largerCollectionWriteMessageData.message.descriptor.dataCid
            )
            ...
            expect(thirdCollectionsQueryReply.status.code).to.equal(200)
            expect(thirdCollectionsQueryReply.entries?.length).to.equal(1)
            expect((thirdCollectionsQueryReply.entries![0] as CollectionsWriteMessage).descriptor.dataCid).to.equal(
                largerCollectionWriteMessageData.message.descriptor.dataCid
            ) // expecting unchanged
        })
        ...
        it('should return 400 if lineageParent is referencing a non-root message', async () => {
            ...
            const rootMessageWriteReply = await handleCollectionsWrite(
                rootMessageData.message,
                messageStore,
                didResolverStub
            )
        ...
        describe('protocol based writes', () => {
            it('should allow write with allow-anyone rule', async () => {
                ...
                const protocolWriteReply = await handleProtocolsConfigure(
                    protocolsConfigureMessageData.message,
                    messageStore,
                ...
                const bobWriteReply = await handleCollectionsWrite(
                    emailMessageDataFromBob.message,
                    messageStore,
                    bobDidResolverStub
                )
                ...
                const bobRecordQueryReply = await handleCollectionsQuery(
                    messageDataForQueryingBobsWrite.message,
                    messageStore,
                    aliceDidResolverStub
                )
                expect(bobRecordQueryReply.status.code).to.equal(200)
                expect(bobRecordQueryReply.entries?.length).to.equal(1)
                expect((bobRecordQueryReply.entries![0] as CollectionsWriteMessage).encodedData).to.equal(
                    base64url.baseEncode(bobData)
                )
            })
            ...
            it('should allow write with recipient rule', async () => {
                ...
                const credentialApplicationContextId =
                    await credentialApplicationMessageData.collectionsWrite.getCanonicalId()

                const credentialApplicationReply = await handleCollectionsWrite(
                    credentialApplicationMessageData.message,
                    messageStore,
                    aliceDidResolverStub
                )
                ...
                const credentialResponseReply = await handleCollectionsWrite(
                    credentialResponseMessageData.message,
                    messageStore,
                    vcIssuerDidResolverStub
                )
                ...
                const messageDataForQueryingCredentialResponse =
                    await TestDataGenerator.generateCollectionsQueryMessage({
                        requester: alice,
                        target: alice,
                ...
                expect((applicationResponseQueryReply.entries![0] as CollectionsWriteMessage).encodedData).to.equal(
                    base64url.baseEncode(encodedCredentialResponse)
                )
            })
            ...
            it('should fail authorization if record schema is not an allowed type for protocol-based CollectionsWrite', async () => {
                ...
                const protocolConfigureReply = await handleProtocolsConfigure(
                    protocolConfigureMessageData.message,
                    messageStore,
                    didResolver
                )
            ...
            it('should look up recipient path with ancestor depth of 2+ (excluding self) in allow rule correctly', async () => {
                ...
                expect(collectionsQueryReply.entries?.length).to.equal(1)
                expect((collectionsQueryReply.entries![0] as CollectionsWriteMessage).descriptor.dataCid).to.equal(
                    fulfillmentMessageData.message.descriptor.dataCid
                )
            })
