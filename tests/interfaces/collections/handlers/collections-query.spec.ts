...
import {
    constructIndexes,
    handleCollectionsWrite,
} from '../../../../src/interfaces/collections/handlers/collections-write.js'
...
describe('handleCollectionsQuery()', () => {
    describe('functional tests', () => {
        ...
        it('should return records matching the query', async () => {
            ...
            const write1Data = await TestDataGenerator.generateCollectionsWriteMessage({
                requester: alice,
                target: alice,
            })
            const write2Data = await TestDataGenerator.generateCollectionsWriteMessage({
                requester: alice,
                target: alice,
                dataFormat,
                schema: 'schema1',
            })
            const write3Data = await TestDataGenerator.generateCollectionsWriteMessage({
                requester: alice,
                target: alice,
                dataFormat,
                schema: 'schema2',
        ...
        it('should omit records that are not published if `dateSort` sorts on `datePublished`', async () => {
            ...
            // insert data
            const publishedWriteReply = await handleCollectionsWrite(
                publishedWriteData.message,
                messageStore,
                didResolverStub
            )
            const unpublishedWriteReply = await handleCollectionsWrite(
                unpublishedWriteData.message,
                messageStore,
                didResolverStub
            )
            ...
            const publishedAscendingQueryReply = await handleCollectionsQuery(
                publishedAscendingQueryData.message,
                messageStore,
                didResolverStub
            )

            expect(publishedAscendingQueryReply.entries?.length).to.equal(1)
            expect(publishedAscendingQueryReply.entries[0].descriptor['datePublished']).to.equal(
                publishedWriteData.message.descriptor.datePublished
            )
            ...
            const publishedDescendingQueryReply = await handleCollectionsQuery(
                publishedDescendingQueryData.message,
                messageStore,
                didResolverStub
            )

            expect(publishedDescendingQueryReply.entries?.length).to.equal(1)
            expect(publishedDescendingQueryReply.entries[0].descriptor['datePublished']).to.equal(
                publishedWriteData.message.descriptor.datePublished
            )
        })
        ...
        it('should sort records if `dateSort` is specified', async () => {
            ...
            const createdAscendingQueryReply = await handleCollectionsQuery(
                createdAscendingQueryData.message,
                messageStore,
                didResolverStub
            )

            expect(createdAscendingQueryReply.entries[0].descriptor['dateCreated']).to.equal(
                write1Data.message.descriptor.dateCreated
            )
            expect(createdAscendingQueryReply.entries[1].descriptor['dateCreated']).to.equal(
                write2Data.message.descriptor.dateCreated
            )
            expect(createdAscendingQueryReply.entries[2].descriptor['dateCreated']).to.equal(
                write3Data.message.descriptor.dateCreated
            )
            ...
            const createdDescendingQueryReply = await handleCollectionsQuery(
                createdDescendingQueryData.message,
                messageStore,
                didResolverStub
            )

            expect(createdDescendingQueryReply.entries[0].descriptor['dateCreated']).to.equal(
                write3Data.message.descriptor.dateCreated
            )
            expect(createdDescendingQueryReply.entries[1].descriptor['dateCreated']).to.equal(
                write2Data.message.descriptor.dateCreated
            )
            expect(createdDescendingQueryReply.entries[2].descriptor['dateCreated']).to.equal(
                write1Data.message.descriptor.dateCreated
            )
            ...
            expect(publishedAscendingQueryReply.entries[0].descriptor['datePublished']).to.equal(
                write1Data.message.descriptor.datePublished
            )
            expect(publishedAscendingQueryReply.entries[1].descriptor['datePublished']).to.equal(
                write2Data.message.descriptor.datePublished
            )
            expect(publishedAscendingQueryReply.entries[2].descriptor['datePublished']).to.equal(
                write3Data.message.descriptor.datePublished
            )
            ...
            expect(publishedDescendingQueryReply.entries[1].descriptor['datePublished']).to.equal(
                write2Data.message.descriptor.datePublished
            )
            expect(publishedDescendingQueryReply.entries[2].descriptor['datePublished']).to.equal(
                write1Data.message.descriptor.datePublished
            )
        })
        ...
        it('should only return published records and unpublished records that is meant for requester', async () => {
            ...
            const privateRecordsForBob = replyToBob.entries.filter(
                message => (message as any).encodedData === Encoder.stringToBase64Url('2')
            )
            const privateRecordsFromBob = replyToBob.entries.filter(
                message => (message as any).encodedData === Encoder.stringToBase64Url('3')
            )
            const publicRecords = replyToBob.entries.filter(
                message => (message as any).encodedData === Encoder.stringToBase64Url('4')
            )
            ...
            const replyToAliceQuery = await handleCollectionsQuery(
                aliceQueryMessageData.message,
                messageStore,
                didResolver
            )
        ...
        it('should not fetch entries across tenants', async () => {
            ...
            const collectionsWriteMessage2Data = await TestDataGenerator.generateCollectionsWriteMessage({
                requester: bob,
                target: bob,
                schema,
            })
