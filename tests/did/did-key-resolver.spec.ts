...
describe('DidKeyResolver', () => {
    it('should resolve a ed25519 `did:key` DID correctly', async () => {
        ...
        expect(
            resolutionDocument.didDocument['@context'].indexOf('https://w3id.org/security/suites/ed25519-2020/v1')
        ).to.not.equal(-1)
        expect(resolutionDocument.didDocument.verificationMethod[0].publicKeyJwk.x).to.equal(
    ...
    it('should resolve a secp256k1 `did:key` DID correctly', async () => {
        ...
        expect(resolutionDocument.didDocument.verificationMethod[0].publicKeyJwk.x).to.equal(
            'RwiZITTa2Dcmq-V1j-5tgPUshOLO31FbsnhVS-7lskc'
        )
        expect(resolutionDocument.didDocument.verificationMethod[0].publicKeyJwk.y).to.equal(
            '3o1-UCc3ABh757P58gDISSc4hOj9qyfSGl3SGGA7xdc'
        )
    })
