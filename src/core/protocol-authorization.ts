...
export class ProtocolAuthorization {
    ...
    public static async authorize(
        ...
        const ancestorMessageChain: CollectionsWriteMessage[] =
            await ProtocolAuthorization.constructAncestorMessageChain(collectionsWrite, messageStore)
    ...
    private static getRuleSet(
        ...
        while (true) {
            ...
            if (!(currentRecordType in allowedRecordsAtCurrentLevel)) {
                throw new Error(
                    `record with schema: '${currentRecordSchema}' not allowed in structure level ${currentMessageIndex}`
                )
            }
    ...
    private static verifyAllowedActions(
        ...
        if (!allowedActions.includes(inboundMessageAction)) {
            throw new Error(
                `inbound message action '${inboundMessageAction}' not in list of allowed actions ${allowedActions}`
            )
        }
    }
    ...
    private static getMessage(
        ...
        if (expectedAncestors.length > ancestorMessageChain.length) {
            throw new Error(
                'specified path to expected recipient is longer than actual length of ancestor message chain'
            )
        }
        ...
        while (true) {
            ...
            if (actualAncestorType !== expectedAncestorType) {
                throw new Error(
                    `mismatching record schema: expecting ${expectedAncestorType} but actual ${actualAncestorType}`
                )
            }
