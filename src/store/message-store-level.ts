...
export class MessageStoreLevel implements MessageStore {
    ...
    // type `SearchIndex` is not exported. So we construct it indirectly from function return type
    ...
    async open(): Promise<void> {
        ...
        // calling `searchIndex()` twice without closing its
        //  DB causes the process to hang(ie.calling this method consecutively),
    ...
    async close(): Promise<void> {
        await this.db.close()
        await this.index.INDEX.STORE.close()
        // MUST close index-search DB, else `searchIndex()` triggered in a different
        // instance will hang indefinitely
    }
    ...
    async put(messageJson: BaseMessage, indexes: { [key: string]: string }): Promise<void> {
        // delete `encodedData` if it exists so `messageJson` is stored without it, `encodedData` will
        // be decoded, chunked and stored separately below
